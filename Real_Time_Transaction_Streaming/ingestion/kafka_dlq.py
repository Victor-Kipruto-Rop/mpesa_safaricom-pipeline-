...
from app.database.models import Transaction, ErrorLog
...
class DeadLetterQueueHandler:
    ...
    def _log_dlq_entry(self, dlq_message: DeadLetterMessage):
        ...
        try:
            ...
            error_log = ErrorLog(
                ...
                recovery_status="pending" if dlq_message.is_recoverable else "unrecoverable",
                metadata={
    ...
    def _consume_dlq_messages(self):
        ...
        for message in self.dlq_consumer:
            try:
                ...
                if dlq_message.is_recoverable and dlq_message.retry_count < dlq_message.max_retries:
    ...
    def get_dlq_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recovery_counts = db.query(
                ErrorLog.recovery_status,
                func.count(ErrorLog.id).label('count')
            ).group_by(ErrorLog.recovery_status).all()
