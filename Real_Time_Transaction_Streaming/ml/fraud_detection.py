...
class FraudDetectionEngine:
    ...
    def extract_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        df['is_very_high_amount'] = (df['amount'] > df['amount'].quantile(0.95)).astype(int)
        df['is_very_low_amount'] = (df['amount'] < df['amount'].quantile(0.05)).astype(int)
        ...
        customer_stats = df.groupby('phone_number').agg({
            'amount': ['mean', 'std', 'min', 'max', 'count'],
            'transaction_time': 'nunique'
        }).reset_index()
        
        customer_stats.columns = ['phone_number', 'customer_avg_amount', 'customer_std_amount',
                                  'customer_min_amount', 'customer_max_amount',
                                  'customer_transaction_count', 'unique_times']
        
        df = df.merge(customer_stats, on='phone_number', how='left')
        
        # Deviation features
        df['amount_deviation'] = np.abs(df['amount'] - df['customer_avg_amount']) / (df['customer_std_amount'] + 1)
        df['amount_deviation'] = df['amount_deviation'].fillna(0)
        
        # Merchant features
        merchant_stats = df.groupby('business_shortcode').agg({
            'amount': ['mean', 'count'],
            'phone_number': 'nunique'
        }).reset_index()
        
        merchant_stats.columns = ['business_shortcode', 'merchant_avg_amount',
                                  'merchant_transaction_count', 'merchant_unique_customers']
        
        df = df.merge(merchant_stats, on='business_shortcode', how='left')
        
        # Regional features
        df['region_risk_factor'] = df['region'].map({
            'Nairobi': 0.3,
            'Central': 0.2,
            'Coast': 0.25,
            'Western': 0.15,
        ...
        feature_cols = [
            'amount_log', 'is_round_amount', 'is_very_high_amount', 'is_very_low_amount',
            'hour_of_day', 'day_of_week', 'is_night_transaction', 'is_weekend',
            'phone_length', 'customer_avg_amount', 'customer_transaction_count',
            'amount_deviation', 'merchant_avg_amount', 'merchant_transaction_count',
            'region_risk_factor'
        ]
    ...
    def train_models(self, days: int = 30):
        ...
        # Label transactions based on isolation forest (1 = fraud, -1 = normal, convert to 1 = fraud, 0 = normal)
        ...
        if y.sum() > 10:  # Need at least 10 positive samples
            ...
            self.random_forest = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)
    ...
    def predict_fraud(self, transaction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            fraud_votes = sum(1 for m in predictions['models'].values() 
    ...
    def batch_fraud_detection(self, days: int = 7) -> Dict[str, Any]:
        ...
        high_risk_customers = df[df['is_suspicious'] == 1].groupby('phone_number').agg({
            'transaction_id': 'count',
            'amount': 'sum',
            'fraud_score': 'mean'
        }).rename(columns={'transaction_id': 'suspicious_count'}).reset_index()
        ...
        return {
            ...
            'high_risk_customers': high_risk_customers[['phone_number', 'suspicious_count', 'amount']].to_dict('records'),
            'timestamp': datetime.utcnow().isoformat()
        }
