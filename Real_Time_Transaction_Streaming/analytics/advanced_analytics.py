...
class AdvancedAnalyticsEngine:
    ...
    def customer_segmentation(self, n_clusters: int = 5) -> Dict[str, Any]:
        ...
        df['volatility'] = df['std_amount'] / (df['avg_amount'] + 1)  # Avoid division by zero
        ...
        features = df[['transaction_count', 'total_amount', 'avg_amount', 'volatility']].copy()
        ...
        return {
            'segments': df[['phone_number', 'segment_name', 'customer_value_score']].to_dict('records'),
    ...
    def anomaly_detection(self, threshold: float = 2.5) -> Dict[str, Any]:
        ...
        # Calculate z-score
        df['amount_zscore'] = np.abs((df['amount'] - df['mean_amount']) / (df['std_amount'] + 1))
        ...
        return {
            ...
            'anomalies': anomalies[['transaction_id', 'phone_number', 'amount', 'amount_zscore']].head(100).to_dict('records'),
            'anomaly_summary': {
    ...
    def regional_analysis(self) -> Dict[str, Any]:
        ...
        df['transaction_percentage'] = (df['transaction_count'] / df['transaction_count'].sum() * 100).round(2)
        df['volume_percentage'] = (df['total_amount'] / df['total_amount'].sum() * 100).round(2)
        
        return {
            'regions': df.to_dict('records'),
            'top_regions': df.nlargest(5, 'total_amount')[['region', 'transaction_count', 'total_amount']].to_dict('records'),
